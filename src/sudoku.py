...
class Sudoku:
    ...
    def _print_usage(self):
        ...
        sys.stderr.write("    To use this program to solve Sudoku puzzles, put the puzzle in a\n")
    ...
    def read(self, file_handle, want_rating):
        try:
            self._sudo = np.genfromtxt(file_handle, delimiter=',', loose=False, dtype='int32')
        except IOError:
            sys.stderr.write("Error: Unable to read file, sorry.\n")
            sys.exit(1)
        except ValueError:
            sys.stderr.write("Error: CSV file should have integers between 1 and 9, inclusive.\n\n")
            self._print_usage()
            sys.exit(1)
    ...
    def _translate_into_constraint_matrix(self):
        ...
        For details of these constraints please see <https://en.wikipedia.org/wiki/Exact_cover#Sudoku>.
        """
        constraints = np.zeros((self._num_possibilities,self._num_constraints), dtype='int32')
